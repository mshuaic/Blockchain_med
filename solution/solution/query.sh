...
if [ -n "$start_time" ]; then command+=" -st $start_time"; fi
if [ -n "$end_time" ]; then command+=" -et $end_time"; fi
if [ -n "$node" ]; then command+=" -node $node"; fi
if [ -n "$id" ]; then command+=" -id $id"; fi
if [ -n "$ref_id" ]; then command+=" -ref $ref_id"; fi
if [ -n "$user" ]; then command+=" -user $user"; fi
if [ -n "$activity" ]; then command+=" -act $activity"; fi
if [ -n "$resource" ]; then command+=" -res $resource"; fi
if [ -n "$sort_by" ]; then command+=" -sort $sort_by"; fi
if [ -n "$order" ]; then command+=" -order $order"; fi
$command
   
# python3 query.py -st $start_time -et $end_time -node $node -id $id \
# 	-ref $ref_id -user $user -act $activity -res $resource \
