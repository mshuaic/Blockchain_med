...
docker run -it -d --name ${dir_prefix}0 -v $dir/multichain:/root/.multichain -p 2750:2750 -p $rpcport:$rpcport $img /usr/sbin/sshd -D 1>/dev/null

# create blockchain
docker exec -d ${dir_prefix}0 multichain-util create $chain -default-rpc-port=$rpcport -default-network-port=$[$rpcport+1]
...
do
    ...
    docker run -it -d --name $nodename -v $(pwd)/$nodename/multichain:/root/.multichain $img /usr/sbin/sshd -D 1>/dev/null
    # docker exec $nodename multichaind $chain@$ip:$conport -daemon
    address=$(docker exec $nodename multichaind $chain@$ip:$conport -daemon | grep connect,send,receive | awk '{print $4}')
    docker exec -d ${dir_prefix}0 multichain-cli $chain grant $address connect,send,receive
    docker exec -d $nodename multichaind $chain@$ip:$conport -daemon
